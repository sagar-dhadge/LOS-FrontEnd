...
import * as LeadAction from './store/lead.action';
...
export class DataService {
  ...
  getRowdata2() {
    this.store.dispatch(
      new LeadAction.GetData({
        dataColumns: this.data2.data.columns,
        dataRows: this.data2.data.rows,
      })
    );
  }
  ...
  updateData1(id: string, value: any) {
    return this.http.put('https://localhost:7058/api/LeadList/' + id, value);
  }
  ...
  // }
  // updateData2(index: number, updatedRow: any) {
  //   this.data2.data.rows[index] = updatedRow;
  //   this.rowDataChanged.next(this.data2.data.rows.slice());
  //   return this.rowDataChanged.asObservable(); // Return the Observable
  // }
